...
export function cycleRecombination(parent1, parent2) {
    ...
    const cycles = [];
    const visited1 = new Array();
    const visited2 = new Array();
    let i = 0;
    while (true) {
        let current_value = parent1[i];
        if (visited1[0] === current_value) {
            cycles.push([visited1, visited2]);
            i = parent1.find((val) => {
                if (!visited1.includes(val)) {
                    return parent1.indexOf(val);
                }
            });
            visited1 = [];
            visited2 = [];
        ...
        visited1.push(current_value);
        let counterpart_value = parent2[i];
        visited2.push(counterpart_value);
        i = counterpart_value;
    }

    for (const cycle of cycles) {
        const [cycle1, cycle2] = cycle;
        for (let i = 0; i < cycle1.length; i++) {
            child1[cycle1[i]] = cycle2[i];
            child2[cycle2[i]] = cycle1[i];
        }
    }
